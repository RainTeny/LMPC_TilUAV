% ===================== TilUAV_plot_all.m ===================== % Independent Plotting Script — Comparison: Tilting Rotor vs. Quadrotor % All attitude angles in degrees. Make sure the following variables are preloaded: % p, xm_vector_tilt, y_vector_tilt, r_mat_tilt, u_vector_tilt % p, xm_vector_4q, y_vector_4q, r_mat_4q, u_vector_4q load('tilrotor_tilt_plotdata.mat'); xm_vector_tilt = xm_vector; y_vector_tilt = y_vector; u_vector_tilt = u_vector; r_mat_tilt = r_mat; %% Convert radians to degrees load quadrotor_4q_plotdata.mat
% 重命名变量（不影响倾转旋翼部分）
...
% 调整图形位置和大小，为外部图例预留空间
set (gcf, 'Position', [100 100 800 600]); % 宽度增加以容纳右侧图例
labels = {'x [m]', 'y [m]', 'z [m]', };
for i = 1:3
subplot (3,1,i);
...
% 图例位置：图形外部右上角
legend ('Tilt Output','Tilt Ref','Quad Output', 'Location', 'northeastoutside');
end
%% 2. Attitude Angles (Roll, Pitch, Yaw)
figure ('Name','Attitude Comparison');
set (gcf, 'Position', [100 100 800 600]); % 调整图形宽度
...
% 图例位置：图形外部右上角
if i == 1
legend ('TiltRotor','Quadrotor', 'Location', 'northeastoutside');
elseif i == 2
legend ('TiltRotor','Quadrotor', 'Location', 'northeastoutside');
elseif i == 3
legend ('TiltRotor','Quadrotor', 'Location', 'northeastoutside');
...
% 图例位置：图形外部右上角
legend (' 实际轨迹 ',' 参考轨迹 ', 'Location', 'northeastoutside');
...
% 图例位置：图形外部右上角
legend ('TiltRotor','Quadrotor','Reference', 'Location', 'northeastoutside');