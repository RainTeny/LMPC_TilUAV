...
% u=[F1, F2, F3, F4, α1, α2 ] F1-4分别是四个机翼升力
% u_=[sinα1*F1、sinα2*F3、cosα1*F1、cosα2*F3、F2、F4 ]
xdot1=x4;
xdot2=x5;
xdot3=x6;
xdot4=(sin(u5)*u1+sin(u6)*u3)*(cos(x8)*cos(x9))/m+(cos(u5)*u1+cos(u6)*u3+u2+u4)*(cos(x7)*sin(x8)*cos(x9)+sin(x7)*sin(x9))/m-kx*x4/m+dx/m;
xdot5=(sin(u5)*u1+sin(u6)*u3)*(cos(x8)*sin(x9))/m+(cos(u5)*u1+cos(u6)*u3+u2+u4)*(cos(x7)*sin(x8)*cos(x9)-sin(x7)*cos(x9))/m-ky*x5/m+dy/m;
xdot6=-(sin(u5)*u1+sin(u6)*u3)*sin(x8)/m+(cos(u5)*u1+cos(u6)*u3+u2+u4)*(cos(x7)*cos(x8))/m-g-kz*x6/m+dz/m;
...
xdot10=(lm*(cos(u5)*u1-cos(u6)*u3)/Ix)-((Iy-Iz)/Ix)*x11*x12;
xdot11=(lm*(u3-u4)/Iy)-((Iz-Ix)/Iy)*x10*x12;
xdot12=(lm*(sin(u5)*u1+sin(u6)*u3)/Iz)+(d*(cos(u5)*u1+cos(u6)*u3-u3-u4)/Iz)-((Ix-Iy)/Iz)*x10*x11;
...
%% 雅可比线性化
% 稳定点
ue=[0.25*m*g,0.25*m*g,0.25*m*g,0.25*m*g,0,0].';
...
for k=1:N_sim 
    ...
    rx(k)=sin(0.03*k);    
    ry(k)=cos(0.03*k);       
%     rx(k) = 2 * k;         % 假设 x 随时间线性增长
%     ry(k) = 3 * (k);    % 假设 y 是时间的余弦函数
%     rz(k) = 1.5 * k.^2;    % 假设 z 是时间的平方函数 
     rz(k)=100; 
%    rz(k)=sin(0.03*k);
    ryaw(k)=45*pi/180;
